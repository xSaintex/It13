...
namespace IT13
{
    public partial class RentalList : Form
    {
        ...
        {
            Filter.Items.Clear();
            Filter.Items.AddRange(new[] { "Filter", "All", "Pending", "Confirmed", "Cancelled", "Completed" });
        ...
        private void Filter_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                string statusFilter = Filter.SelectedItem.ToString();
                LoadDataFromDatabase(statusFilter);
            }
        }
        ...
        private void LoadDataFromDatabase(string statusFilter = null)
        {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = @"
                        SELECT 
                            ...
                            c.PhoneNo,
                            COALESCE(c.Add1 + ', ' + c.City, c.Add1, c.City, 'N/A') AS Address,
                            COUNT(ri.RentalItemID) AS ItemCount,
                            r.total,
                            r.status
                        FROM rentals r
                        LEFT JOIN customers c ON r.CustomerID = c.CustID
                        LEFT JOIN rental_items ri ON r.RentalID = ri.RentalID
                        WHERE 1=1";
                    ...
                    query += @"
                        GROUP BY 
                            r.RentalID, c.CompanyName, c.FirstName, c.LastName, 
                            c.PhoneNo, c.Add1, c.City, r.total, r.status
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                string rentalId = "RENT-" + reader["RentalID"].ToString().PadLeft(6, '0');
                                string customerName = reader["CustomerName"]?.ToString() ?? "N/A";
                                string phone = reader["PhoneNo"]?.ToString() ?? "N/A";
                                ...
                                AddRow(rentalId, customerName, phone, address, itemCount,
                                       $"₱{total:N2}", status);
                            }
                        }
                    }
            ...
            {
                MessageBox.Show($"Error loading rentals: {ex.Message}", "Database Error",
        ...
        private void dgvRentals_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (e.ColumnIndex == dgvRentals.Columns["colStatus"].Index && e.RowIndex >= 0)
            {
                ...
                Color bg = status.ToLower() switch
                {
                    ...
                    "confirmed" => Color.FromArgb(0, 123, 255),
