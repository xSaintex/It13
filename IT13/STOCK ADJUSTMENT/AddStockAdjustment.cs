...
namespace IT13
{
    public partial class AddStockAdjustment : Form
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = @"
                        ...
                        (StockItemID, RequestedBy, ReviewedBy, RequestedDate, ReviewedDate, 
                         Reason, AdjustmentType, PhysicalCount, SystemCount, AdjustCount, Status)
                        VALUES 
                        (@StockItemID, @RequestedBy, @ReviewedBy, @RequestedDate, @ReviewedDate,
                         @Reason, @AdjustmentType, @PhysicalCount, @SystemCount, @AdjustCount, @Status)";
            ...
            {
                MessageBox.Show($"Error saving stock adjustment: {ex.Message}", "Database Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
